---
title: "Exercicio de Estimação e simulação de Monte-Carlo com modelo ARMA(1,1)-GARCH(1,1)"
---
...
distribuicao=3  # 1 distribuição normal
...
ARMAGARCH <- function(params,data,np,nq,nr,ns,nu) {
  ...
  #Distribuição t-Student
    ...
    const1 <- (gamma((nu+1)*0.5))/gamma((nu)*0.5)
    verossim <- log(const1) - 0.5*log(pi*(nu-2)) - 0.5*(log(sigma2)) - 0.5*(nu+1)*(log(1+(errors))^(2)/(sigma2)*(nu-2))
    return(list(LLF=-sum(verossim),sigma2=sigma2,residuals=errors/sqrt(sigma2)))
...
ARMAGARCH.optim <- function(params,data,np,nq,nr,ns,nu) {
  ARMAGARCH(params,data,np,nq,nr,ns,nu)$LLF
}
```
...
```{r, echo=FALSE}
# Estima modelo
resultados.finais2 <- garchFit(formula = ~ arma(1,1) + garch(1, 1), data = returns[,1], cond.dist = c("std"), include.mean = F)

# Compara a verossimilhança obtida com código próprio e com pacote do R
LLF.meu.codigo <- print(-ARMAGARCH(resultados$par,data=returns[,1],np=1,nq=1,nr=1,ns=1, nu=2)$LLF)
LLF.fGarch <- print(-ARMAGARCH(c(0.0206345,-0.0062683,0.2994673,0.0862977,0.7732459),data=returns[,1],np=1,nq=1,nr=1,ns=1, nu=2)$LLF)
```
...
MC.ARMAGARCH <- function(N.sim) {
  param.matrix <- matrix(ncol=6,nrow=N.sim)
  colnames(param.matrix) <- c("AR","MA","omega","alpha","beta","nu")
  for (i in 1:N.sim) {
    set.seed(i+2)
    print(i)
    sim.data <- ARMAGARCH.simulate(1000,0.2,-0.1,0.2,0.05,0.85)$data
    param.sim <- constrOptim(init.params,ARMAGARCH.optim,data=sim.data,np=1,nq=1,nr=1,ns=1, nu=3,grad=NULL,ui=ui,ci=ci)$par
